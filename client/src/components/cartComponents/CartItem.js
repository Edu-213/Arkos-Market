          ...
          <div className="flex flex-col gap-[0.5rem] p-[1rem]">
            <div className="flex items-start">
              ...
              <div className="flex flex-col gap-[0.25rem] ml-[0.25rem] mr-auto">
                <div className="flex flex-col items-start gap-[0.25rem]">
                  ...
                  <a href={`/produto/${item.product.slug}`} className="cart-Item-text text-xs leading-[1rem] hover:underline">
                    {item.product.name}
                  </a>
                </div>
              </div>
              <button className="uppercase font-bold justify-center items-center" type="button" onClick={removeFromCart}>
                <span className="text-red-600 text-xs items-center">
                  {' '}
            ...
            <div className="flex justify-between gap-[0.5rem] items-start">
              <div className="flex flex-col gap-[0.5rem] text-left">
                ...
                <p className="text-base font-bold text-[#00FF00] leading-[1.75rem]">
          ...
          <div className="grid grid-cols-[5.5rem,2fr,1fr] gap-[1rem] py-[1rem]">
            ...
            <div className="flex flex-col">
              <div className="flex flex-col justify-start">
                ...
                <a href={`/produto/${item.product.slug}`} className="text-sm cart-Item-text leading-[1.125rem] hover:underline">
                  {item.product.name}
                </a>
              </div>
              <div className="cts text-xs pt-[8px]">
                <p>
                  Parcelado no cartão sem juros: <b>{formatCurrency(item.product.finalPrice)}</b>
                </p>
              </div>
              ...
              <div>
                <div className="grid grid-cols-[1fr,auto,1fr] gap-[0.25rem] items-end">
                  ...
                  <div className="flex flex-col justify-center items-center gap-[0.1rem]">
                    <label>
                      <span className="cts items-center text-sm">Quant.</span>
                    </label>
                    <input
                      value={item.quantity}
                      onChange={handleQuantityChange}
                      min="1"
                      max={item.product.maxPurchesedLimit}
                      className="leading-[1rem] font-bold text-center flex max-w-[2.875rem] text-base"
                    />
                  </div>
                  <button onClick={() => updateQuantity(item.quantity + 1)} disabled={item.quantity >= item.product.maxPurchesedLimit}>
                    <svg width="10" height="14" viewBox="0 0 10 14" fill="none" xmlns="https://www.w3.org/2000/svg" aria-hidden="true">
                ...
                <div className="pt-[10px]">
                  <button className="flex uppercase font-bold justify-center items-center" type="button" onClick={removeFromCart}>
                    <span className="text-red-600 text-xs items-center">
                      <svg width="24" height="32" viewBox="0 0 24 32" fill="none" xmlns="https://www.w3.org/2000/svg" className="w-[1rem] h-[1rem]" aria-hidden="true">
                        <path
                          fill-rule="evenodd"
                    ...
                    <span className="text-xs text-red-600 items-center">remover</span>
                  </button>
                </div>
              </div>
              <div>
                <div className="gap-[0.625rem] flex flex-col">
                  <div>
                    <p className="text-[#42464D] text-sm ">Preço a vista no pix: </p>
                  </div>
                  <div>
                    <p className="text-base font-bold text-[#00FF00] text-end leading-[1.75rem]">
                      {formatCurrency((item.product.finalPriceWithPix || item.product.finalPrice) * item.quantity)}
                    </p>
                  </div>
                </div>
