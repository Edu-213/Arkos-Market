...
router.post('/add', verifyToken, async (req, res) => {
    ...
    try {
        ...
        let cart = await Cart.findOneAndUpdate({user: req.user.id, 'items.product' : productId}, { $inc: { 'items.$.quantity': quantity}}, { new: true});
        if (!cart) {
            cart = await Cart.findOneAndUpdate({ user: req.user.id }, { $push: { items: { product: productId, quantity } } }, { new: true, upsert: true } );
        }
